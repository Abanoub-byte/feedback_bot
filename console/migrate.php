...
echo sprintf("existed %s migration%s", count($migrations), PHP_EOL);
...
echo sprintf("max migration $maxver%s", PHP_EOL);

foreach ($migrations as $migration) {
    $migration->setDatabase($database);
    if ($migration->getVersion()>$maxver) {
        echo sprintf("accept migration %s%s", $migration->getVersion(), PHP_EOL);
        $migration->up();
    }
}
